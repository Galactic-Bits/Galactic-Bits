...
# GalacticBits Multi-installer
# a one line clone-and-compile for GalacticBits:
#
#     ` $ curl -sL "https://raw.githubusercontent.com/GalacticBits/GalacticBits/master/scripts/multi_installer.sh" | bash
#
...
_set_wd() {
    if [ -d "$PWD/.git" ] && [ -f "$PWD/Makefile" ]; then
        ...
        if [ -d "$PWD"/GalacticBits ]; then
            read -r -p "${1:-GalacticBits directory already exists. Overwrite? [y/N]} " response
            case "$response" in
                [yY][eE][sS|[yY])
                    _colorize red "Overwriting old GalacticBits directory" && echo
                    rm -rf "$PWD"/GalacticBits
                    ;;
                *)
                    _fail "GalacticBits directory already exists. Aborting..."
                    ;;
            esac
        fi
        mkdir GalacticBits
        git clone -b master -q https://github.com/GalacticBits/GalacticBits GalacticBits   >>build.log 2>&1 || _fail "Unable to clone git repository. Please see build.log for more information"
        cd GalacticBits
    fi
}

_build_GalacticBits() {
    _note "Building GalacticBits from source (this might take a while)..."
    if [ -d build ]; then
...
_configure_linux() {
    if [ "$(awk -F= '/^NAME/{print $2}' /etc/os-release)" = "\"Ubuntu\"" ]; then
        ...
        _fail "Your OS version isn't supported by this installer. Please consider adding support for your OS to the project ('https://github.com/GalacticBits')"
    fi
}
...
_configure_os() {
    ...
    case "${_unameOut}" in
        ...
        Darwin*)
            ...
            ;;
        *)
            _fail "This installer only runs on OSX 10.10+ and Ubuntu 16.04+. Please consider adding support for your OS to the project ('https://github.com/GalacticBits')"
            ;;
    esac
    _note "Operating system configuration completed. You're halfway there!"
}

_note "GalacticBits Multi_Installer v1.0 (pepperoni)"
...
_note "Look in 'GalacticBits/build/src/' for the executible binaries. See 'https://github.com/GalacticBits/GalacticBits' for more project support. Cowabunga!"
